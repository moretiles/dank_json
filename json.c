...
JSON_Node *jsonReadnd(JSON_Node *root) {
  if (root == NULL) {
    return NULL;
  }
...
void interface_tests() {
  ...
  JSON_Node *exists = jsonReadnsl(object, _KEY("D"), _INDEX(2), NULL);
  assert(exists != NULL);
  ...
  JSON_Node *pre = jsonReadnsl(obj_copy, _KEY("D"), _INDEX(3), NULL);
